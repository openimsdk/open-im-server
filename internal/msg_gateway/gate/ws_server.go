...
func (ws *WServer) wsHandler(w http.ResponseWriter, r *http.Request) {
	...
	if isPass, compression := ws.headerCheck(w, r, operationID); isPass {
		...
		if err != nil {
			...
			conn.add
			newConn := &UserConn{conn, new(sync.Mutex), utils.StringToInt32(query["platformID"][0]), 0, compression, query["sendID"][0], false, query["token"][0], conn.LocalAddr().String() + "_" + conn.RemoteAddr().String() + "_" + strconv.Itoa(int(utils.GetCurrentTimestampBySecond()))}
			userCount++

...
func (p *Pusher) DeleteMemberAndSetConversationSeq(ctx context.Context, groupID string, userIDs []string) error {
	...
	maxSeq, err := p.msgRPCClient.GetConversationMaxSeq(ctx, conversationID)
	if err != nil {
		return err
	}
	return p.conversationRPCClient.SetConversationMaxSeq(ctx, userIDs, conversationID, maxSeq)
}

...
type CommonMsgDatabase interface {
	...
	SearchMessage(ctx context.Context, sendID string, recvID string, sendTime string, msgType int32, sessionType int32) (msgData []*sdkws.MsgData, err error)
...
func (db *commonMsgDatabase) SearchMessage(ctx context.Context, sendID string, recvID string, sendTime string, msgType int32, sessionType int32) (msgData []*sdkws.MsgData, err error) {
	var totalMsgs []*sdkws.MsgData
	msgs, err := db.msgDocDatabase.SearchMessage(ctx, sendID, recvID, sendTime, msgType, sessionType)
	if err != nil {
		return nil, err
	}
	for _, msg := range msgs {
